...
import (
	...
	"github.com/google/battery-historian/db"
)
...
//func (pd *ParsedData) SaveToDb(w http.ResponseWriter, r *http.Request) {
//
//}
...
func (pd *ParsedData) SendAsJSON(w http.ResponseWriter, r *http.Request) {
	...
	db.InsertRecord(db.TaskRecord{
		SDKVersion:pd.data[0].SDKVersion,
		DeviceId:pd.data[0].DeviceID,
		DeviceModel: pd.data[0].DeviceModel,
		AnalyzedData: string(unzipped),
...
func HTTPAnalyzeHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if part.FileName() == "" {
			...
			if err != nil {
				http.Error(w, "Failed to read field 'remark'. Please try again.", http.StatusInternalServerError)
				return
			}
			r.Form[part.FormName()]= []string{ string(b) }
			continue
		}
